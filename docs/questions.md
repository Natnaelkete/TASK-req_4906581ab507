...
This file is mandatory. For every area of ambiguity in the HarborClass
prompt it records:

- **Question:** what is unclear or unspecified in the prompt
- **Assumption:** our interpretation of the intended behaviour
- **Solution:** how we implemented it based on that assumption
...
- **Assumption:** Students should not be blocked entirely; their cancel should enter a `pending_approval` state so a teacher (or admin) can later approve it via the same endpoint.
